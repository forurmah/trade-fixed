...
import { DecisionFormData, FormErrors, ChosenOptionValue, AddDecisionFormProps } from '../types';
import { INITIAL_DECISION_FORM } from '../constants';
import {
...
export const AddDecisionForm: React.FC<AddDecisionFormProps> = ({ onSave, onCancel }) => {
  const [formData, setFormData] = useState<DecisionFormData>(INITIAL_DECISION_FORM);
  const [errors, setErrors] = useState<FormErrors>({});
  const [storageError, setStorageError] = useState<string | null>(null);
  const [hasAttemptedSubmit, setHasAttemptedSubmit] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // References to form fields to move keyboard focus on validation failure
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      // Pass validated trimmed payload to parent handler
      onSave({
      ...
      // Reset local state for next entry only if save succeeded without throwing
      setFormData(INITIAL_DECISION_FORM);
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Unable to save decision. Local storage may be full or disabled.';
  ...
  return (
    ...
    >
        ...
        >
          <div className="flex items-start gap-2.5 min-w-0">
            ...
            <div className="min-w-0 break-words [overflow-wrap:anywhere]">
              <p className="font-semibold text-[#801B33]">Unable to save decision</p>
      ...
      <div className="min-w-0">
        ...
        />
        {errors.title && (
          <p id="title-error" className="mt-1.5 text-[13px] font-medium text-[#9E2D46] break-words [overflow-wrap:anywhere]">
            {errors.title}
          </p>
        )}
      </div>

      {/* 2. Side-by-side Options (stacked on mobile) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <input
            ...
            onChange={handleChange}
            placeholder="Finish my React project"
            aria-describedby={errors.optionA ? 'option-a-error' : undefined}
          ...
          />
          {errors.optionA && (
            <p id="option-a-error" className="mt-1.5 text-[13px] font-medium text-[#9E2D46] break-words [overflow-wrap:anywhere]">
              {errors.optionA}
            </p>
          )}
        </div>
        ...
        <div className="min-w-0">
          ...
          <input
            ...
            onChange={handleChange}
            placeholder="Meet my friends"
            aria-describedby={errors.optionB ? 'option-b-error' : undefined}
          ...
          />
          {errors.optionB && (
            <p id="option-b-error" className="mt-1.5 text-[13px] font-medium text-[#9E2D46] break-words [overflow-wrap:anywhere]">
              {errors.optionB}
            </p>
          )}
        </div>
      ...
      {/* 3. Radio-button group with stable values 'A' and 'B' */}
      <fieldset
        className="space-y-2.5"
        aria-describedby={errors.chosenOption ? 'chosen-option-error' : undefined}
      >
        ...
        <div className="space-y-2 pt-1">
          ...
          >
            <div className="relative flex items-center justify-center">
              <input
                ...
                type="radio"
                id="radio-choice-option-a"
                name="chosenOptionGroup"
                value="A"
          ...
          {/* Option B Radio */}
          <label
            htmlFor="radio-choice-option-b"
            className="flex items-center gap-3 cursor-pointer select-none py-1 group"
          >
            <div className="relative flex items-center justify-center">
              <input
                type="radio"
                id="radio-choice-option-b"
                name="chosenOptionGroup"
                value="B"
        ...
        {errors.chosenOption && (
          <p id="chosen-option-error" className="text-[13px] font-medium text-[#9E2D46]">
            {errors.chosenOption}
          </p>
        )}
      </fieldset>

      {/* 4. Full-width multiline textarea */}
      <div className="min-w-0">
        <div className="flex items-center justify-between mb-1.5">
          <label
        ...
        <textarea
          ...
          onChange={handleChange}
          placeholder="I want to finish the project before university starts."
          aria-describedby={
            errors.reason ? 'reason-error reason-help-text' : 'reason-help-text'
          }
        ...
        <div className="flex items-center justify-between mt-1.5 gap-2">
          <p id="reason-help-text" className="text-[13px] text-[#4F596F]">
            A short, honest reason is enough.
          </p>
        </div>
        {errors.reason && (
          <p id="reason-error" className="mt-1.5 text-[13px] font-medium text-[#9E2D46] break-words [overflow-wrap:anywhere]">
            {errors.reason}
          </p>
        )}
      </div>
